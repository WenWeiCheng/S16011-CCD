...
* @file proto_num.c
*
* Protocol number/set parsing and formatting implementation (pure logic, host-testable
* standalone).
*
* @note <pre>
...
void Proto_FmtFloat(char *buf, u32 cap, float v, int dec)
{
    ...
    if (dec > 0) {
        ...
        if (fp >= scale) { fp -= scale; ip++; }   /* fractional carry */
    } else {
        fp = 0L;
    }
    ...
    if (dec > 0) {
        ...
        for (z = m; z < (u32)dec; z++) {          /* pad leading zeros */
            if (n + 1U < cap) buf[n++] = '0';
        }
        for (z = 0U; z < m; z++) {
...
{
    u32 i, cnt = Proto_EnumCount(list);
    char buf[32];

    for (i = 0; i < cnt; i++) {
        if (Proto_EnumLabel(list, i, buf, sizeof buf) == 0 &&
            strcmp(buf, tok) == 0) {
            *out = (s32)i;
            return 0;
        }
